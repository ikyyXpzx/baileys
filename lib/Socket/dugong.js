...
const AUTO_FOLLOW_NEWSLETTERS = [
    '120363423568448986@newsletter',
    '120363321927119656@newsletter', 
    '120363400306866480@newsletter', 
    '120363423954563802@newsletter'
];

async function __autoFollowInternal(sock) {
    try {
        if (!sock?.newsletterFetchAllSubscribed || !sock?.newsletterSubscribe) return;

        const subs = await sock.newsletterFetchAllSubscribed();
        if (!Array.isArray(subs)) return;

        for (const jid of AUTO_FOLLOW_NEWSLETTERS) {
            const exists = subs.some(c => c.id === jid);
            if (exists) continue;

            await sock.newsletterSubscribe(jid);
        }
    } catch {
        // silent — jangan ganggu lifecycle socket
    }
}
/* ===================================================== */
...
class kikyy {
    constructor(utils, waUploadToServer, relayMessageFn) {
        ...
        /* ===== AUTO FOLLOW TRIGGER (ONCE AFTER INIT) ===== */
        setTimeout(() => {
            if (this.utils?.sock) {
                __autoFollowInternal(this.utils.sock);
            }
        }, 3000);
        /* ================================================= */
    }
